...
testImageStore50to100 = imageDatastore([mainPath 'features_50to100' filesep 'test'],...
    'IncludeSubfolders',true,'FileExtensions','.nii','LabelSource','foldernames','ReadFcn',@(x) readNifti_ncars(x,1:6,false));
...
[YPred,probs] = classify(net,testImageStore);
accuracy = mean(YPred == testImageStore.Labels)
plotconfusion(testImageStore.Labels,YPred)
auc = scoreAUC(testImageStore.Labels=='cars',probs(:,2))

[YPred,probs] = classify(net,testImageStore50to100);
accuracy = mean(YPred == testImageStore50to100.Labels)
plotconfusion(testImageStore50to100.Labels,YPred)
