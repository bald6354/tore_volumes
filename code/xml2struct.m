    ...
    try 
        if exist(input, 'file') == 2
            ...
            try
                ...
                %sometimes there are errors reading from string - write to
                %file and then read - wes (idk why this happens, it appears
                %that a few key/value pairs have very long entries
                try
                    if exist('test.xml','file')
                        delete('test.xml')
                    end
